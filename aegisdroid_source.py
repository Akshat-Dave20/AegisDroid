#!/usr/bin/env python3
# AegisDroid v1.0 - Created by Akshat Dave
...
"""
HEADER_LINES = [
    " AegisDroid v-1.0 | Created by Akshat Dave",
...
def option_backup_apk():
    device = get_selected_device()
    if device is None:
        return
    package = input("Enter package name to backup/extract (e.g., com.example.app): ").strip()
    if not package:
        print(RED + "[-] Package name is required." + RESET)
        return
    print(CYAN + f"[*] Finding APK path for {package}..." + RESET)
    out, err = run_cmd(f"adb -s {device} shell pm path {package}", capture=True)
    if not out or "package:" not in out:
        print(RED + f"[-] Could not find APK for package '{package}'." + RESET)
        print(YELLOW + "Tip: Make sure the package name is correct and the app is installed." + RESET)
        return
    apk_path = out.replace("package:", "").strip()
    local_filename = f"{package}.apk"
    print(CYAN + f"[*] Pulling APK from {apk_path}..." + RESET)
    out_pull, err_pull = run_cmd(f"adb -s {device} pull {apk_path} ./{local_filename}", capture=True)
    if os.path.isfile(local_filename):
        print(GREEN + f"[+] APK extracted and saved as ./{local_filename}" + RESET)
    else:
        print(RED + "[-] Failed to pull APK from device:" + RESET)
        print(err_pull or out_pull)
...
def show_menu():
    ...
    print(CYAN + f"[23] View Device Connection History\n")
    print(CYAN + f"[24] Backup/Extract Installed APK" + RESET)
    print("\n")
...
def main():
    ...
    while True:
        ...
        print_banner()
        show_menu()
        choice = input(GREEN + "Select an option (1-24) or q to quit: " + RESET).strip().lower()
        if choice == "q":
            ...
            print(CYAN + "Exiting AegisDroid. Stay ethical." + RESET)
            break
        options = {
            ...
            "24": option_backup_apk
        }
        if choice in options:
            options[choice]()
        else:
            print(RED + "Invalid option. Choose 1-24 or q." + RESET)
